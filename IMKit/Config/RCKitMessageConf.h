...
#pragma mark MessageNotificaiton
/*!
 *  \~chinese
 是否关闭所有的本地通知，默认值是NO
 ...
 *  \~english
 ...
 @ discussion The default value is 60 seconds.
 */
 ...
 *  \~english
 Whether APP monopolizes audio.

 @ discussion The default value is NO and AVAudioconversation's setActive:NO is will be called after recording ends.
 Restore the sound played by other backend APP. If it is set to YES, setActive:NO will not be called. Thus, the sound played by the current APP will not be interrupted.
 ...
 *  \~english
 Whether to include video files when selecting media resources. The default value is NO.
 ...
 *  \~english
 Automatic download size of GIF message , in KB.
 */
 ...
 *  \~english
 Whether to enable merge forwarding. The default value is NO. Afer it is enabled, you can merge and forward messages  (currently, only single chat and group chat are supported).
 */
 ...
 *  \~english
 Whether the message reference function is supported. The default value is YES. Chat page holding message supports references  (currently, only text message, file message, image message, image message and reference message are supported).
*/
 ...
 *  \~english
 Whether to enable automatic message resending. The default value is YES.

 @ discussion When it is enabled, SDK will automatically resend when the message fails to be sent.
*/
 ...
 *  \~english
 The time limit for uploading videos (in seconds), which is equal to [RCCoreClient getVideoDurationLimit].
*/
