...
///
/// 未注册的消息，可以通过 ``RCConversationViewController/rcUnkownConversationCollectionView:cellForItemAtIndexPath:`` 和 ``RCConversationViewController/rcUnkownConversationCollectionView:layout:sizeForItemAtIndexPath:`` 方法定制在会话页面的显示。
/// 未注册的消息，可以通过修改 `unknown_message_cell_tip` 字符串资源定制在会话列表界面的显示。
...
/// 针对这种情况，可以预先定义好未注册的消息的显示，以提升用户体验（如提示当前版本不支持，引导用户升级版本等）
///
/// 未注册的消息，可以通过修改 `unknown_message_notification_tip` 字符串资源定制本地通知的显示。
///
...
///
/// 默认值是 60 秒
/// - Warning: 该接口已被废弃，最大长度只支持 60 秒的语音消息
@property (nonatomic, assign) NSUInteger maxVoiceDuration __deprecated_msg();

/// APP是否独占音频
///
/// 默认是 NO, 录音结束之后会调用 `AVAudioSession` 的 `setActive:NO` ，
/// 恢复其他后台APP播放的声音，如果设置成 YES, 不会调用 setActive:NO，这样不会中断当前APP播放的声音
