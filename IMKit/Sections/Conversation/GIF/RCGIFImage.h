...
extern const NSTimeInterval kRCGIFImageDelayTimeIntervalMinimum;

//
//  An `RCGIFImage`'s job is to deliver frames in a highly performant way and works in conjunction with
//  `RCGIFImageView`.
...
// Pass either a `UIImage` or an `RCGIFImage` and get back its size
+ (CGSize)sizeForImage:(id)image;

// On success, the initializers return an `RCGIFImage` with all fields initialized, on failure they return `nil` and an
...
typedef NS_ENUM(NSUInteger, RCGIFImageLogLevel) {
    RCGIFImageLogLevelNone = 0,
    RCGIFImageLogLevelError,
    RCGIFImageLogLevelWarn,
    RCGIFImageLogLevelInfo,
    RCGIFImageLogLevelDebug,
    RCGIFImageLogLevelVerbose
};

@interface RCGIFImage (Logging)
...
#define RCLog(logLevel, format, ...)                                                                                   \
    [RCGIFImage logStringFromBlock:^NSString * {                                                                       \
        return [NSString stringWithFormat:(format), ##__VA_ARGS__];                                                    \
    }                                                                                                                  \
                         withLevel:(logLevel)]

@interface RCGIFImageWeakProxy : NSProxy
