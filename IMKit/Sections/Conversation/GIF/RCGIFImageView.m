...
@optional
- (void)debug_animatedImageView:(RCGIFImageView *)animatedImageView
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<RCGIFImageViewDebugDelegate> debug_delegate;
#endif

@end

@implementation RCGIFImageView
...
- (void)setAnimatedImage:(RCGIFImage *)animatedImage {
...
- (NSTimeInterval)frameDelayGreatestCommonDivisor {
    // Presision is set to half of the `kRCGIFImageDelayTimeIntervalMinimum` in order to minimize frame dropping.
    const NSTimeInterval kGreatestCommonDivisorPrecision = 2.0 / kRCGIFImageDelayTimeIntervalMinimum;
...
- (void)startAnimating {
    if (self.animatedImage) {
        ...
        if (!self.displayLink) {
            ...
            RCGIFImageWeakProxy *weakProxy = [RCGIFImageWeakProxy weakProxyForObject:self];
...
- (void)displayDidRefresh:(CADisplayLink *)displayLink {
    ...
    // Early return!
    if (!self.shouldAnimate) {
        RCLog(RCGIFImageLogLevelWarn, @"Trying to animate image when we shouldn't: %@", self);
        return;
    }
    ...
    if (delayTimeNumber != nil) {
        ...
        if (image) {
            RCLog(RCGIFImageLogLevelVerbose, @"Showing frame %lu for animated image: %@",
            ...
            RCLog(RCGIFImageLogLevelDebug, @"Waiting for frame %lu for animated image: %@",
                  (unsigned long)self.currentFrameIndex, self.animatedImage);
#if defined(DEBUG) && DEBUG
