...
- (void)edit_refreshNormalInputReferenceViewIfNeeded:(NSArray<RCMessageModel *> *)messageModels {
    ...
    RCReferenceInputBarView *referenceView = [self currentReferenceInputBarView];
    RCMessageModel *referenceModel = [self currentReferenceInputBarMessageModel];
    if (messageModelDict.count == 0 || !referenceView || !referenceModel) {
        return;
    }
    NSString *referencedMessageUId = referenceModel.messageUId;
    ...
    [self performOnMainThread: ^{
        [referenceView setReferencedMessageModel:model];
        if (self.referencingView == referenceView) {
