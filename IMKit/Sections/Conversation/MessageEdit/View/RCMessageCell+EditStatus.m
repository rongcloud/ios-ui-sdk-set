...
@implementation RCMessageCell (EditStatus)
...
- (void)edit_showEditStatus:(RCMessageModifyStatus)editStatus {
    [self edit_updateEditStatus:editStatus];
}
...
+ (CGFloat)edit_editStatusBarHeightWithModel:(RCMessageModel *)model {
    if (model.modifyInfo && model.modifyInfo.status != RCMessageModifyStatusSuccess) {
