...
- (void)viewDidLoad {
    ...
    self.offsettest = 0.0;
    self.ContentOffset = self.currentIndex * (currentViewSize.width);
...
- (void)viewDidLayoutSubviews{
    ...
    currentViewSize = self.view.frame.size;
    self.scrollView.frame = CGRectMake(0, 0, (currentViewSize.width), currentViewSize.height);
    [self.scrollView setContentSize:CGSizeMake([self.imageArray count] * (currentViewSize.width), 0)];
    [self.scrollView setContentOffset:CGPointMake((currentViewSize.width)*self.currentIndex, 0)];
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    self.scrollView.contentOffset = CGPointMake(self.currentIndex * (currentViewSize.width), 0);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    self.currentIndex = scrollView.contentOffset.x / (currentViewSize.width);
    ...
    if (self.newContentOffsetX < self.ContentOffset && (self.currentIndex == 0 || self.currentIndex == 1)) {
        ...
        if ([imageMessageForward count] > 0) {
            ...
            //设置ContentSize
            [self.scrollView setContentSize:CGSizeMake([imageArr count] * (currentViewSize.width), 0)];
            //更新当前图片的索引和坐标
            self.currentIndex = [imageArr count] - 2;
            self.preSelectIndex = self.currentIndex;
            [scrollView setContentOffset:CGPointMake(self.newContentOffsetX +
                                                         imageMessageForward.count * (currentViewSize.width),
                                                     0)];
        ...
        if ([imageMessagebackward count] > 0) {
            ...
            [self.scrollView setContentOffset:CGPointMake((currentViewSize.width), 0)];
            self.currentImageView = self.imageViewList[self.currentIndex];
            //                  }
        }
    }
...
- (void)doubleTap:(UITapGestureRecognizer *)sender {
    ...
    if (subview.contentSize.width > currentViewSize.width) {
        ...
        CGFloat xsize = currentViewSize.width / newZoomScale;
        CGFloat ysize = currentViewSize.height / newZoomScale;
...
- (void)resizeSubviews:(RCloudImageView *)ImageView {
    ...
    CGFloat width = currentViewSize.width;
    CGFloat height = currentViewSize.height;
        ...
        for (UIView *subView in imageView.subviews) {
            if ([subView isKindOfClass:[RCImageMessageProgressView class]] && subView.hidden == NO) {
                if (CGPointEqualToPoint(imageView.center, self.view.center)) {
                    ...
                    [subView setCenter:CGPointMake(currentViewSize.width / 2, currentViewSize.height / 2)];
                }
                break;
            }
        }
        ...
        if (imageMessage) {
            ...
            if (imageMessage.localPath.length > 0 &&
                ...
                if ([imageMessage.imageUrl hasPrefix:@"http"]) {
                    ...
                    [imageProressView
                        setCenter:CGPointMake(currentViewSize.width / 2, currentViewSize.height / 2)];
    ...
    for (int i = 0; i < imageViewList.count; i++) {
        ...
        if (i == imageViewList.count - 1) {
            imagesrcoll = [[UIScrollView alloc]
                initWithFrame:CGRectMake(i * (currentViewSize.width), 0, currentViewSize.width,
        ...
        [imagesrcoll setContentSize:CGSizeMake(currentViewSize.width, currentViewSize.height)];

        imagesrcoll.delegate = self;

        UIImageView *imageView = imageViewList[i];
        RCloudImageView *newImageView = [[RCloudImageView alloc]
            initWithFrame:CGRectMake(0, 0, currentViewSize.width, currentViewSize.height)];
...
- (void)action:(NSTimer *)scheduledTimer {
    ...
    if ([imageUrl hasPrefix:@"http"]) {
        ...
        [imageViewTip setCenter:CGPointMake(currentViewSize.width / 2, currentViewSize.height / 2)];
        [imageView addSubview:imageViewTip];
        UILabel *failLabel = [[UILabel alloc] initWithFrame:CGRectMake(currentViewSize.width / 2 - 75,
                                                                       currentViewSize.height / 2 + 44, 150, 30)];
        ...
        [imageView addSubview:imageViewTip];
        UILabel *failLabel =
            [[UILabel alloc] initWithFrame:CGRectMake(currentViewSize.width / 2 - 75,
                                                      currentViewSize.height / 2 + 49.5, 150, 30)];
...
- (UIScrollView *)scrollView{
    if (!_scrollView) {
        ...
        [_scrollView setContentSize:CGSizeMake([self.imageArray count] * (currentViewSize.width), 0)];
        if ([RCKitUtility isRTL]) {
            [_scrollView setTransform:CGAffineTransformMakeScale(-1, 1)];
        }
        [_scrollView setContentSize:CGSizeMake([self.imageArray count] * (currentViewSize.width), 0)];
        [_scrollView setContentOffset:CGPointMake((currentViewSize.width)*self.currentIndex, 0)];
        [self refreshimage:self.imageArray];
        [self refreshScrollView:self.imageViewList];
    }
    return _scrollView;
