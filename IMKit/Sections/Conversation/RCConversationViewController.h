...
/// 
/// 默认是 10，传入 `1 < count <= 100` 之间的数值。
/// 此属性需要在viewDidLoad之前进行设置。
@property (nonatomic, assign) int defaultMessageCount;


/// 设置进入会话页面后下拉刷新从远端获取消息的条数。
/// 默认是 10，`defaultRemoteHistoryMessageCount` 传入 `1 < count <= 100` 之间的数值。
/// 此属性需要在viewDidLoad之前进行设置。
...
/// 加载消息类型（不支持聊天室）
///
/// 加载消息是先从本地获取历史消息，本地有缺失的情况下会从服务端同步缺失的部分。 从服务端同步失败的时候会返回非 0 的 errorCode，同时把本地能取到的消息回调上去。
/// 此属性可以控制从服务端同步失败的时候加载消息的方式。
/// - Since: 5.1.7
...
/// - Parameter messageContent: 消息的内容
/// - Parameter pushContent:    接收方离线时需要显示的远程推送内容
/// - Parameter appUpload:      是否上传到App指定的服务器
...
/// 如果您通过sendMediaMessage:pushContent:appUpload:接口发送媒体消息，则必须实现此回调。
...
/// 取消上传媒体消息。
/// - Parameter model:        媒体消息（文件消息）的Model
/// 如果您通过sendMediaMessage:pushContent:appUpload:发送媒体消息（上传媒体内容到App服务器），需要
/// 重写此函数，在此函数中取消掉您的上传，并调用uploadListener的cancelBlock告诉融云SDK该发送已经取消。目前仅支持文件消息的取消
- (void)cancelUploadMedia:(RCMessageModel *)model;

/// 重新发送消息
/// - Parameter messageContent: 消息的内容
/// 发送消息失败，点击小红点时，会将本地存储的原消息实体删除，回调此接口将消息内容重新发送。
...
/// 重新发送消息
/// - Parameter model: 消息的 Model
/// 发送消息失败，点击小红点时，会将原消息移动到会话页面最下方，不删除原消息，直接重新发送该消息。
...
/// 通过此方法插入一条消息，会将消息实体对应的内容Model插入数据源中，并更新UI。
...
/// 注册自定义消息的Cell
/// - Parameter cellClass:     自定义消息cell的类
/// - Parameter messageClass:  自定义消息Cell对应的自定义消息的类，该自定义消息需要继承于RCMessageContent
...
/// - Parameter newImage:    图片
/// 您可以在此回调中按照您的需求，将图片另行保存或执行其他操作。
- (void)saveNewPhotoToLocalSystemAfterSendingSuccess:(UIImage *)newImage;

/// 查看位置信息的位置详情
/// - Parameter locationMessageContent:  点击的位置消息
...
/// 点击公众号菜单
/// - Parameter selectedMenuItem:  被点击的公众号菜单
- (void)onPublicServiceMenuItemSelected:(RCPublicServiceMenuItem *)selectedMenuItem;

/// 点击公众号Cell中的URL的回调
/// - Parameter url:   被点击的URL
/// - Parameter model: 被点击的Cell对应的Model
...
/// 应用可以重写此方法来自定义客服评价界面。应用不要直接调用此方法。
/// - Parameter serviceStatus:  当前的服务类型。
/// - Parameter commentId: 评论ID。当是用户主动离开客服会话时，这个id是null；当客服在后台推送评价请求时，这个id是对话id；当用户点击机器人应答评价时，这个是机器人知识库id。
/// - Parameter isQuit:         评价完成后是否离开
...
/// 选择客服分组
/// - Parameter groupList:    所有客服分组
/// - Parameter resultBlock:  resultBlock
/// 重写这个方法你可以自己重写客服分组界面。
/// 当用户选择技能组后，调用resultBlock传入用户选择分组的groupId，如果用户没有选择，可以传nil，会自动分配一个客服分组
...
/// 客服服务模式变化
/// - Parameter newMode:  新的客服服务模式。
- (void)onCustomerServiceModeChanged:(RCCSModeType)newMode;

/// 客服通告
/// - Parameter announceMsg:  客服通告内容
/// - Parameter announceClickUrl:  客服通告链接url
...
/// - Parameter selectedBlock: 选人后的回调
/// - Parameter cancelBlock:   取消选人的回调
...
/// 合并转发消息的回调
/// - Parameter index:            0 是逐条转发消息, 1 是合并转发消息。
