...
static NSUInteger const RC_KIT_USER_PROFILE_BATCH_FETCH_SIZE = 20;
static NSTimeInterval const RC_KIT_PRELOAD_INFO_MISS_RETRY_INTERVAL = 30.0;
...
/// 预加载用户信息未命中时间，避免未命中数据被高频事件反复触发
@property (nonatomic, strong) NSMutableDictionary<NSString *, NSNumber *> *preloadUserMissTimestamps;

/// 预加载群成员信息未命中时间，key 格式：groupId_userId
@property (nonatomic, strong) NSMutableDictionary<NSString *, NSNumber *> *preloadGroupMemberMissTimestamps;
...
- (void)loadUserInfos:(NSArray<NSString *> *)userIds {
    ...
    // 4. 过滤近期预加载未命中的用户，再标记正在请求中的，直接调用批量拉取（依赖通知更新，不使用回调）
    NSArray *userIdsToPreload = [self p_filterPreloadMissItems:missingUserIdsSet.allObjects
                                                    timestamps:self.preloadUserMissTimestamps
                                                          lock:self.userFetchingLock];
    NSArray *userIdsToFetch = [self p_filterAndMarkFetchingUserIds:userIdsToPreload];
    if (userIdsToFetch.count > 0) {
        [self p_batchFetchUserInfos:userIdsToFetch recordPreloadMiss:YES complete:nil];
    }
}
...
- (void)fetchUserInfos:(NSArray<NSString *> *)userIds {
    ...
    if (userIdsToFetch.count > 0) {
        [self p_batchFetchUserInfos:userIdsToFetch recordPreloadMiss:NO complete:nil];
    }
}

- (void)clearUserInfo:(NSString *)userId {
    [self.cache removeUserCache:userId];
    [self p_clearPreloadMissItems:@[ userId ?: @"" ]
                       timestamps:self.preloadUserMissTimestamps
                             lock:self.userFetchingLock];
}
...
- (void)clearAllUserInfo {
    ...
    [self.userFetchingLock performWriteLockBlock:^{
        [self.preloadUserMissTimestamps removeAllObjects];
    }];
}
...
- (void)preloadGroupMembers:(NSArray<NSString *> *)userIds 
    ...
    // 4. 过滤近期预加载未命中的群成员，再标记正在请求中的，直接调用批量拉取
    NSArray *userIdsToPreload = [self p_filterPreloadMissGroupMemberUserIds:missingUserIdsSet.allObjects
                                                                     groupId:groupId];
    NSArray *userIdsToFetch = [self p_filterAndMarkFetchingGroupMemberKeys:userIdsToPreload
                                                                    groupId:groupId];
    if (userIdsToFetch.count > 0) {
        [self p_batchFetchGroupMembers:userIdsToFetch groupId:groupId recordPreloadMiss:YES complete:nil];
    }
}
...
- (void)fetchGroupMembers:(NSArray<NSString *> *)userIds withGroupId:(NSString *)groupId {
    ...
    if (userIdsToFetch.count > 0) {
        [self p_batchFetchGroupMembers:userIdsToFetch groupId:groupId recordPreloadMiss:NO complete:nil];
    }
}
...
- (void)clearGroupMember:(NSString *)userId inGroup:(NSString *)groupId {
    ...
    [self p_clearPreloadMissGroupMemberUserIds:@[ userId ?: @"" ] groupId:groupId];
}

- (void)clearAllGroupMember {
    [self.cache removeAllGroupMemberCache];
    [self.memberFetchingLock performWriteLockBlock:^{
        [self.preloadGroupMemberMissTimestamps removeAllObjects];
    }];
}
...
- (void)p_processBatchItems:(NSArray *)allItems
    ...
    [self p_processBatchItems:allItems
           ...
           accumulatedResults:accumulatedResults
                    batchSize:RC_KIT_BATCH_FETCH_SIZE
...
- (void)p_processBatchItems:(NSArray *)allItems
    ...
    // 计算当前批次范围
    NSUInteger safeBatchSize = MAX(batchSize, 1);
    NSUInteger endIndex = MIN(startIndex + safeBatchSize, allItems.count);
...
- (NSArray<NSString *> *)p_filterPreloadMissItems:(NSArray<NSString *> *)items
                                       timestamps:(NSMutableDictionary<NSString *, NSNumber *> *)timestamps
                                             lock:(RCIMThreadLock *)lock {
    if (items.count == 0) {
        return @[];
    }
    NSTimeInterval now = NSDate.date.timeIntervalSince1970;
    NSMutableOrderedSet<NSString *> *itemsToPreload = [NSMutableOrderedSet orderedSet];
    [lock performWriteLockBlock:^{
        NSMutableArray<NSString *> *expiredKeys = [NSMutableArray array];
        [timestamps enumerateKeysAndObjectsUsingBlock:^(NSString *key, NSNumber *lastPreloadTime, BOOL *stop) {
            if (now - lastPreloadTime.doubleValue >= RC_KIT_PRELOAD_INFO_MISS_RETRY_INTERVAL) {
                [expiredKeys addObject:key];
            }
        }];
        if (expiredKeys.count > 0) {
            [timestamps removeObjectsForKeys:expiredKeys];
        }

        for (NSString *item in items) {
            if (item.length == 0 || timestamps[item]) {
                continue;
            }
            [itemsToPreload addObject:item];
        }
    }];
    return itemsToPreload.array;
}

- (void)p_recordPreloadMissItems:(NSArray<NSString *> *)items
                       timestamps:(NSMutableDictionary<NSString *, NSNumber *> *)timestamps
                             lock:(RCIMThreadLock *)lock {
    if (items.count == 0) {
        return;
...
- (void)p_clearPreloadMissItems:(NSArray<NSString *> *)items
                     timestamps:(NSMutableDictionary<NSString *, NSNumber *> *)timestamps
                           lock:(RCIMThreadLock *)lock {
    if (items.count == 0) {
        return;
    ...
    [lock performWriteLockBlock:^{
        [timestamps removeObjectsForKeys:items];
    }];
}

- (NSArray<NSString *> *)p_filterPreloadMissGroupMemberUserIds:(NSArray<NSString *> *)userIds
                                                       groupId:(NSString *)groupId {
    if (userIds.count == 0 || groupId.length == 0) {
        return @[];
    }
    NSTimeInterval now = NSDate.date.timeIntervalSince1970;
    NSMutableOrderedSet<NSString *> *userIdsToPreload = [NSMutableOrderedSet orderedSet];
    [self.memberFetchingLock performWriteLockBlock:^{
        NSMutableArray<NSString *> *expiredKeys = [NSMutableArray array];
        [self.preloadGroupMemberMissTimestamps enumerateKeysAndObjectsUsingBlock:^(NSString *key, NSNumber *lastPreloadTime, BOOL *stop) {
            if (now - lastPreloadTime.doubleValue >= RC_KIT_PRELOAD_INFO_MISS_RETRY_INTERVAL) {
                [expiredKeys addObject:key];
            }
        }];
        if (expiredKeys.count > 0) {
            [self.preloadGroupMemberMissTimestamps removeObjectsForKeys:expiredKeys];
        }
        ...
        for (NSString *userId in userIds) {
            ...
            if (self.preloadGroupMemberMissTimestamps[key]) {
                continue;
            }
            [userIdsToPreload addObject:userId];
        }
    }];
    return userIdsToPreload.array;
}

- (void)p_recordPreloadMissGroupMemberUserIds:(NSArray<NSString *> *)userIds
                                      groupId:(NSString *)groupId {
    if (userIds.count == 0 || groupId.length == 0) {
        return;
    }
    ...
    [self.memberFetchingLock performWriteLockBlock:^{
        for (NSString *userId in userIds) {
            if (userId.length > 0) {
                NSString *key = [NSString stringWithFormat:@"%@_%@", groupId, userId];
                self.preloadGroupMemberMissTimestamps[key] = @(now);
            }
        }
    }];
}

- (void)p_clearPreloadMissGroupMemberUserIds:(NSArray<NSString *> *)userIds
                                     groupId:(NSString *)groupId {
    if (userIds.count == 0 || groupId.length == 0) {
        return;
    }
    NSMutableArray<NSString *> *keys = [NSMutableArray arrayWithCapacity:userIds.count];
    for (NSString *userId in userIds) {
        if (userId.length > 0) {
            [keys addObject:[NSString stringWithFormat:@"%@_%@", groupId, userId]];
        }
    }
    [self.memberFetchingLock performWriteLockBlock:^{
        [self.preloadGroupMemberMissTimestamps removeObjectsForKeys:keys];
    }];
}
...
- (void)p_batchFetchGroupMembers:(NSArray<NSString *> *)userIds
                         groupId:(NSString *)groupId
               recordPreloadMiss:(BOOL)recordPreloadMiss
    ...
    [self p_processBatchItems:userIds
        ...
        [self p_batchGetGroupMembers:batchUserIds
            ...
            for (RCGroupMemberInfo *memberInfo in groupMembers) {
                ...
                if (memberInfo.userId.length > 0) {
                    [foundUserIds addObject:memberInfo.userId];
                }
                [accumulatedResults addObject:user];
            }

            if (recordPreloadMiss) {
                NSArray *foundIds = foundUserIds.allObjects;
                [self p_clearPreloadMissGroupMemberUserIds:foundIds groupId:groupId];
                NSMutableSet *missUserIds = [NSMutableSet setWithArray:batchUserIds];
                [missUserIds minusSet:foundUserIds];
                [self p_recordPreloadMissGroupMemberUserIds:missUserIds.allObjects groupId:groupId];
            }
...
- (void)p_batchFetchUserInfos:(NSArray<NSString *> *)userIds
            recordPreloadMiss:(BOOL)recordPreloadMiss
                     complete:(nullable void (^)(NSArray<RCUserInfo *> *users))complete {
    if (userIds.count == 0) {
        if (complete) {
            complete(@[]);
        }
    ...
    [self p_processBatchItems:userIds
        ...
        [self p_batchGetFriendsInfo:batchUserIds
            ...
            if (recordPreloadMiss) {
                [self p_clearPreloadMissItems:foundUserIds.allObjects
                                   timestamps:self.preloadUserMissTimestamps
                                         lock:self.userFetchingLock];
            }
            ...
            if (notFoundUserIds.count > 0) {
                [self p_batchGetUserProfiles:notFoundUserIds 
                    ...
                    // 处理获取到的用户信息
                    NSMutableSet *profileUserIds = [NSMutableSet set];
                    for (RCUserProfile *profile in profiles) {
                        ...
                        if (profile.userId.length > 0) {
                            [profileUserIds addObject:profile.userId];
                        }
                        [accumulatedResults addObject:user];
                    }

                    if (recordPreloadMiss) {
                        [self p_clearPreloadMissItems:profileUserIds.allObjects
                                           timestamps:self.preloadUserMissTimestamps
                                                 lock:self.userFetchingLock];
                        NSMutableSet *missUserIds = [NSMutableSet setWithArray:notFoundUserIds];
                        [missUserIds minusSet:profileUserIds];
                        [self p_recordPreloadMissItems:missUserIds.allObjects
                                            timestamps:self.preloadUserMissTimestamps
                                                  lock:self.userFetchingLock];
                    }
...
- (void)p_batchGetUserProfiles:(NSArray<NSString *> *)userIds 
    ...
    if (userIds.count > RC_KIT_USER_PROFILE_BATCH_FETCH_SIZE) {
        NSMutableArray *accumulatedProfiles = [NSMutableArray array];
        [self p_processBatchItems:userIds
                       startIndex:0
               accumulatedResults:accumulatedProfiles
                        batchSize:RC_KIT_USER_PROFILE_BATCH_FETCH_SIZE
                   batchProcessor:^(NSArray *batchUserIds, NSUInteger batchStart, NSUInteger batchEnd, void (^continueNextBatch)(void)) {
            [self p_batchGetUserProfiles:batchUserIds retryCount:retryCount complete:^(NSArray<RCUserProfile *> *profiles) {
                [accumulatedProfiles addObjectsFromArray:profiles ?: @[]];
                continueNextBatch();
            }];
...
- (NSMutableDictionary<NSString *, NSNumber *> *)preloadUserMissTimestamps {
    if (!_preloadUserMissTimestamps) {
        _preloadUserMissTimestamps = [NSMutableDictionary dictionary];
    }
    return _preloadUserMissTimestamps;
}

- (NSMutableDictionary<NSString *, NSNumber *> *)preloadGroupMemberMissTimestamps {
    if (!_preloadGroupMemberMissTimestamps) {
        _preloadGroupMemberMissTimestamps = [NSMutableDictionary dictionary];
    }
    return _preloadGroupMemberMissTimestamps;
}
