...
@interface RCInfoProvider () <RCUserInfoUpdateDelegate, RCConversationUserInfoUpdateDelegate,
...
@implementation RCInfoProvider

+ (instancetype)sharedManager {
    static RCInfoProvider *defaultManager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (!defaultManager) {
            defaultManager = [[RCInfoProvider alloc] init];
...
- (void)onConversationUserInfoUpdate:(RCUserInfo *)userInfo
    ...
    if (conversationType == ConversationType_GROUP && userInfo.userId) {
        [RCInfoNotificationCenter postGroupMemberUpdateNotification:userInfo groupId:targetId];
    }
}
...
- (void)onConversationInfoUpdate:(RCConversationInfo *)conversationInfo {
    if (conversationInfo.conversationType == ConversationType_GROUP && conversationInfo.targetId) {
        ...
        [RCInfoNotificationCenter postGroupUpdateNotification:cacheGroupInfo];
    }
}
        ...
        if (!cacheProfile &&
            ...
            dispatch_async(self.requestQueue, ^{
                [[RCIM sharedRCIM]
                    ...
                    [RCInfoNotificationCenter postPublicServiceUpdateNotification:profile forServiceId:serviceId];
                                 }];
            });
        } else {
