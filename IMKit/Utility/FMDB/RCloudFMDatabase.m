...
static int RCloudFMDBDatabaseBusyHandler(void *f, int count) {
    RCloudFMDatabase *self = (__bridge RCloudFMDatabase *)f;
    ...
    if (delta < [self maxBusyRetryTimeInterval]) {
        ...
        if (actualSleepInMilliseconds != requestedSleepInMillseconds) {
            NSLog(@"WARNING: Requested sleep of %i milliseconds, but SQLite returned %i. Maybe SQLite wasn't built "
                  @"with HAVE_USLEEP=1?",
...
int RCloudFMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names); // shhh clang.
int RCloudFMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names) {
...
void RCloudFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc,
                                           sqlite3_value **argv); // -Wmissing-prototypes
void RCloudFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
...
@implementation RCloudFMDatabase
...
#pragma mark RCloudFMDatabase instantiation and deallocation

+ (instancetype)databaseWithPath:(NSString *)aPath {
    return RCloudFMDBReturnAutoreleased([[self alloc] initWithPath:aPath]);
}
...
- (void)dealloc {
    [self close];
    RCloudFMDBRelease(_openResultSets);
    RCloudFMDBRelease(_cachedStatements);
    RCloudFMDBRelease(_dateFormat);
    RCloudFMDBRelease(_databasePath);
    RCloudFMDBRelease(_openFunctions);
...
// if ([RCloudFMDatabase RCloudFMDBVersion] >= 0x0240) { … }
...
+ (SInt32)RCloudFMDBVersion {
    ...
    static SInt32 RCloudFMDBVersionVal = 0;

    dispatch_once(&once, ^{
        NSString *prodVersion = [self RCloudFMDBUserVersion];
        ...
        char *e = nil;
        RCloudFMDBVersionVal = (int)strtoul([junk UTF8String], &e, 16);

    });

    return RCloudFMDBVersionVal;
}
...
- (void)setMaxBusyRetryTimeInterval:(NSTimeInterval)timeout {
    ...
    if (timeout > 0) {
        sqlite3_busy_handler(_db, &RCloudFMDBDatabaseBusyHandler, (__bridge void *)(self));
    } else {
...
// but for folks who don't bother noticing that the interface to RCloudFMDatabase changed,
...
- (void)resultSetDidClose:(RCloudFMResultSet *)resultSet {
...
- (RCloudFMStatement *)cachedStatementForQuery:(NSString *)query {

    NSMutableSet *statements = [_cachedStatements objectForKey:query];

    return [[statements objectsPassingTest:^BOOL(RCloudFMStatement *statement, BOOL *stop) {
...
- (void)setCachedStatement:(RCloudFMStatement *)statement forQuery:(NSString *)query {
    ...
    [_cachedStatements setValue:statements forKey:query];

    RCloudFMDBRelease(query);
}
...
+ (NSDateFormatter *)storeableDateFormat:(NSString *)format {
    ...
    NSDateFormatter *result = RCloudFMDBReturnAutoreleased([[NSDateFormatter alloc] init]);
    result.dateFormat = format;
    result.timeZone = [NSTimeZone timeZoneForSecondsFromGMT:0];
    result.locale = RCloudFMDBReturnAutoreleased([[NSLocale alloc] initWithLocaleIdentifier:@"en_US"]);
    return result;
}
...
- (void)setDateFormat:(NSDateFormatter *)format {
    RCloudFMDBAutorelease(_dateFormat);
    _dateFormat = RCloudFMDBReturnRetained(format);
}
...
- (BOOL)goodConnection {
    ...
    RCloudFMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    NSLog(@"The RCloudFMDatabase %@ is currently in use.", self);

#ifndef NS_BLOCK_ASSERTIONS
    if (_crashOnErrors) {
        NSAssert(false, @"The RCloudFMDatabase %@ is currently in use.", self);
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        NSLog(@"The RCloudFMDatabase %@ is not open.", self);

#ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSAssert(false, @"The RCloudFMDatabase %@ is not open.", self);
            abort();
        }
#endif
...
- (NSError *)errorWithMessage:(NSString *)message {
    ...
    return [NSError errorWithDomain:@"RCloudFMDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];
}
...
- (RCloudFMResultSet *)executeQuery:(NSString *)sql
    ...
    RCloudFMStatement *statement = 0x00;
    RCloudFMResultSet *rs = 0x00;
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            RCloudFMDBRelease(parameterName);
    ...
    RCloudFMDBRetain(statement); // to balance the release below

    if (!statement) {
        statement = [[RCloudFMStatement alloc] init];
    ...
    rs = [RCloudFMResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (RCloudFMResultSet *)executeQueryWithFormat:(NSString *)format, ... {
    va_list args;
...
- (RCloudFMResultSet *)executeQuery:(NSString *)sql values:(NSArray *)values error:(NSError *__autoreleasing *)error {
    RCloudFMResultSet *rs = [self executeQuery:sql withArgumentsInArray:values orDictionary:nil orVAList:nil];
    if (!rs && error) {
...
- (BOOL)executeUpdate:(NSString *)sql
 ...
 withArgumentsInArray:(NSArray *)arrayArgs
    ...
    int rc = 0x00;
    sqlite3_stmt *pStmt = 0x00;
    RCloudFMStatement *cachedStmt = 0x00;
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[RCloudFMStatement alloc] init];

        [cachedStmt setStatement:pStmt];

        [self setCachedStatement:cachedStmt forQuery:sql];

        RCloudFMDBRelease(cachedStmt);
    }
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(RCloudFMDBExecuteStatementsCallbackBlock)block {
    ...
    rc = sqlite3_exec([self sqliteHandle], [sql UTF8String], block ? RCloudFMDBExecuteBulkSQLCallback : nil,
...
- (void)makeFunctionNamed:(NSString *)name
    ...
    id b = RCloudFMDBReturnAutoreleased([block copy]);
 ...
 * called, so we use _openFunctions to store the values instead. */
    ...
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (void *)b,
                            &RCloudFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
...
@implementation RCloudFMStatement
