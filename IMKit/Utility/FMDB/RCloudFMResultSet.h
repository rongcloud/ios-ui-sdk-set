...
@class RCloudFMStatement;

/** Represents the results of executing a query on an `<RCloudFMDatabase>`.
...
@interface RCloudFMResultSet : NSObject {
    RCloudFMDatabase *_parentDB;
    RCloudFMStatement *_statement;
...
/** `RCloudFMStatement` used by result set. */

@property (atomic, retain) RCloudFMStatement *statement;

///------------------------------------
/// @name Creating and closing database
///------------------------------------

/** Create result set from `<RCloudFMStatement>`

 @param statement A `<RCloudFMStatement>` to be performed
