...
+ (instancetype)resultSetWithStatement:(RCloudFMStatement *)statement usingParentDatabase:(RCloudFMDatabase *)aDB {
    ...
    return RCloudFMDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    RCloudFMDBRelease(_columnNameToIndexMap);
...
- (NSDictionary *)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return RCloudFMDBReturnAutoreleased([dict copy]);
    } else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"RCloudFMDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
        }
    } else {
        // wtf?
...
- (void)setParentDB:(RCloudFMDatabase *)newDb {
    _parentDB = newDb;
}
