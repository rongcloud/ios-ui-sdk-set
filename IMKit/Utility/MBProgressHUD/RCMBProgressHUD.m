...
#define MBMainThreadAssert()                                                                                           \
    NSAssert([NSThread isMainThread], @"RCMBProgressHUD needs to be accessed on the main thread.");

CGFloat const RCMBProgressMaxOffset = 1000000.f;

static const CGFloat MBDefaultPadding = 4.f;
static const CGFloat MBDefaultLabelFontSize = 16.f;
static const CGFloat MBDefaultDetailsLabelFontSize = 14.f;

@interface RCMBProgressHUD () {
    // Deprecated
...
@interface RCMBProgressHUDRoundedButton : UIButton
@end

@implementation RCMBProgressHUD
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    RCMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (RCMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (RCMBProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = RCMBProgressHUDAnimationFade;
    _mode = RCMBProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)hideAnimated:(BOOL)animated {
    ...
    if (self.minShowTime > 0.0 && self.showStarted) {
        ...
        if (interv < self.minShowTime) {
            NSTimer *timer = [NSTimer timerWithTimeInterval:(self.minShowTime - interv)
                                                     target:self
...
- (void)animateIn:(BOOL)animatingIn
         withType:(RCMBProgressHUDAnimation)type
       completion:(void (^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == RCMBProgressHUDAnimationZoom) {
        type = animatingIn ? RCMBProgressHUDAnimationZoomIn : RCMBProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == RCMBProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == RCMBProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == RCMBProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == RCMBProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    RCMBProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<RCMBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    RCMBBackgroundView *backgroundView = [[RCMBBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = RCMBProgressHUDBackgroundStyleSolidColor;
    ...
    RCMBBackgroundView *bezelView = [RCMBBackgroundView new];
    ...
    UIButton *button = [RCMBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[RCMBRoundProgressView class]];

    RCMBProgressHUDMode mode = self.mode;
    if (mode == RCMBProgressHUDModeIndeterminate) {
    ...
    } else if (mode == RCMBProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[RCMBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    } else if (mode == RCMBProgressHUDModeDeterminate || mode == RCMBProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[RCMBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == RCMBProgressHUDModeAnnularDeterminate) {
            [(RCMBRoundProgressView *)indicator setAnnular:YES];
        }
    } else if (mode == RCMBProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    } else if (mode == RCMBProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[RCMBProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[ [RCMBProgressHUD class] ]];
#endif

        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[RCMBRoundProgressView class]]) {
        RCMBRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [RCMBRoundProgressView appearanceWhenContainedIn:[RCMBProgressHUD class], nil];
#else
        appearance = [RCMBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[ [RCMBProgressHUD class] ]];
#endif
        if (appearance.progressTintColor == nil) {
            ((RCMBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((RCMBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[RCMBBarProgressView class]]) {
        RCMBBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [RCMBBarProgressView appearanceWhenContainedIn:[RCMBProgressHUD class], nil];
#else
        appearance = [RCMBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[ [RCMBProgressHUD class] ]];
#endif
        if (appearance.progressColor == nil) {
            ((RCMBBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((RCMBBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    RCMBBackgroundView *bezelView = self.bezelView;
    if (![bezelView respondsToSelector:@selector(addMotionEffect:)])
        return;
...
- (void)updateConstraints {
    ...
    NSMutableArray *subviews = [NSMutableArray
        arrayWithObjects:self.topSpacer, self.label, self.detailsLabel, self.button, self.bottomSpacer, nil];
    if (self.indicator)
    ...
    [sideConstraints
        addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:@"|-(>=margin)-[bezel]-(>=margin)-|"
                                                                    options:0
                                                                    metrics:metrics
                                                                      views:NSDictionaryOfVariableBindings(bezel)]];
    [sideConstraints
        addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-(>=margin)-[bezel]-(>=margin)-|"
                                                                    options:0
                                                                    metrics:metrics
                                                                      views:NSDictionaryOfVariableBindings(bezel)]];
    ...
    [subviews enumerateObjectsUsingBlock:^(UIView *view, NSUInteger idx, BOOL *stop) {
        ...
        [bezelConstraints
            addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:@"|-(>=margin)-[view]-(>=margin)-|"
                                                                        options:0
                                                                        metrics:metrics
        ...
        if (idx > 0) {
            // Has previous
            NSLayoutConstraint *padding = [NSLayoutConstraint constraintWithItem:view
                                                                       attribute:NSLayoutAttributeTop
                                                                       relatedBy:NSLayoutRelationEqual
...
- (void)setMode:(RCMBProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == RCMBProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
- (void)setNSProgressDisplayLinkEnabled:(BOOL)enabled {
    // We're using CADisplayLink, because NSProgress can change very quickly and observing it may starve the main
    // thread, so we're refreshing the progress only every frame draw
    if (enabled && self.progressObject) {
        // Only create if not already active.
        if (!self.progressObjectDisplayLink) {
            self.progressObjectDisplayLink =
                [CADisplayLink displayLinkWithTarget:self selector:@selector(updateProgressFromProgressObject)];
        }
    } else {
...
@implementation RCMBRoundProgressView
...
@implementation RCMBBarProgressView
...
- (void)setProgressRemainingColor:(UIColor *)progressRemainingColor {
    ...
    if (progressRemainingColor != _progressRemainingColor &&
        ![progressRemainingColor isEqual:_progressRemainingColor]) {
...
@implementation RCMBBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = RCMBProgressHUDBackgroundStyleBlur;
            ...
            _style = RCMBProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(RCMBProgressHUDBackgroundStyle)style {
    if (style == RCMBProgressHUDBackgroundStyleBlur &&
        kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = RCMBProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    RCMBProgressHUDBackgroundStyle style = self.style;
    if (style == RCMBProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == RCMBProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation RCMBProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [RCMBProgressHUD allHUDsForView:view];
    for (RCMBProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (BOOL)dimBackground {
    RCMBBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor = [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == RCMBProgressHUDBackgroundStyleSolidColor &&
           [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    MBMainThreadAssert();
    self.backgroundView.style = RCMBProgressHUDBackgroundStyleSolidColor;
...
@implementation RCMBProgressHUDRoundedButton
