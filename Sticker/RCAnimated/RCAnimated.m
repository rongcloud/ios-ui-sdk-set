...
const NSTimeInterval kRCAnimatedDelayTimeIntervalMinimum = 0.02;
...
// `RCAnimatedDataSizeCategoryAll` category.
typedef NS_ENUM(NSUInteger, RCAnimatedDataSizeCategory) {
    RCAnimatedDataSizeCategoryAll = 10, // All frames permanently in memory (be nice to the CPU)
    RCAnimatedDataSizeCategoryDefault =
        75, // A frame cache of default size in memory (usually real-time performance and keeping low memory profile)
    RCAnimatedDataSizeCategoryOnDemand =
        250, // Only keep one frame at the time in memory (easier on memory, slowest performance)
    RCAnimatedDataSizeCategoryUnsupported // Even for one frame too large, computer says no.
};

typedef NS_ENUM(NSUInteger, RCAnimatedFrameCacheSize) {
    RCAnimatedFrameCacheSizeNoLimit = 0,   // 0 means no specific limit
    RCAnimatedFrameCacheSizeLowMemory = 1, // The minimum frame cache size; this will produce frames on-demand.
    RCAnimatedFrameCacheSizeGrowAfterMemoryWarning = 2, // If we can produce the frames faster than we consume, one
                                                        // frame ahead will already result in a stutter-free playback.
    RCAnimatedFrameCacheSizeDefault = 5                 // Build up a comfy buffer window to cope with CPU hiccups etc.
};

#if defined(DEBUG) && DEBUG
@protocol RCAnimatedDebugDelegate <NSObject>
@optional
- (void)debug_animatedImage:(RCAnimated *)animatedImage didUpdateCachedFrames:(NSIndexSet *)indexesOfFramesInCache;
- (void)debug_animatedImage:(RCAnimated *)animatedImage didRequestCachedFrame:(NSUInteger)index;
- (CGFloat)debug_animatedImagePredrawingSlowdownFactor:(RCAnimated *)animatedImage;
@end
#endif

@interface RCAnimated ()
...
// The actual type of the object is `RCWeakProxy`.
@property (nonatomic, strong, readonly) RCAnimated *weakProxy;

#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<RCAnimatedDebugDelegate> debug_delegate;
#endif
...
- (NSUInteger)frameCacheSizeCurrent {
    ...
    if (self.frameCacheSizeMax > RCAnimatedFrameCacheSizeNoLimit) {
        frameCacheSizeCurrent = MIN(frameCacheSizeCurrent, self.frameCacheSizeMax);
    }

    if (self.frameCacheSizeMaxInternal > RCAnimatedFrameCacheSizeNoLimit) {
        ...
        [[NSNotificationCenter defaultCenter]
            addObserverForName:UIApplicationDidReceiveMemoryWarningNotification
                    ...
                    usingBlock:^(NSNotification *note) {
                        ...
                        // Note that RCAnimateds can be created on any thread, so the hash table must be locked.
                        NSArray *images = nil;
...
- (instancetype)init {
    RCAnimated *animatedImage = [self initWithAnimatedGIFData:nil];
    if (!animatedImage) {
        RCLog(RCAnimatedLogLevelError, @"Use `-initWithAnimatedGIFData:` and supply the animated GIF data as an "
                                       @"argument to initialize an object of type `RCAnimated`.");
    }
...
- (instancetype)initWithAnimatedGIFData:(NSData *)data
    ...
    if (!hasData) {
        RCLog(RCAnimatedLogLevelError, @"No animated GIF data supplied.");
        return nil;
    }
    ...
    if (self) {
        ...
        if (!_imageSource) {
            RCLog(RCAnimatedLogLevelError, @"Failed to `CGImageSourceCreateWithData` for animated GIF data %@", data);
            return nil;
        }
        ...
        if (!isGIFData) {
            RCLog(RCAnimatedLogLevelError, @"Supplied data is of type %@ and doesn't seem to be GIF data %@",
            ...
            @autoreleasepool {
                ...
                if (frameImageRef) {
                    ...
                    if (frameImage) {
                        ...
                        if (delayTime == nil) {
                            if (i == 0) {
                                RCLog(RCAnimatedLogLevelInfo, @"Falling back to default delay time for first frame %@ "
                                                              @"because none found in GIF properties %@",
                                      frameImage, frameProperties);
                                delayTime = @(kDelayTimeIntervalDefault);
                            } else {
                                RCLog(RCAnimatedLogLevelInfo, @"Falling back to preceding delay time for frame %zu %@ "
                                                              @"because none found in GIF properties %@",
                                      i, frameImage, frameProperties);
                                delayTime = delayTimesForIndexesMutable[@(i - 1)];
                            }
                        }
                        // Support frame delays as low as `kRCAnimatedDelayTimeIntervalMinimum`, with anything below
                        ...
                        if ([delayTime floatValue] < ((float)kRCAnimatedDelayTimeIntervalMinimum - FLT_EPSILON)) {
                            RCLog(RCAnimatedLogLevelInfo,
                                  @"Rounding frame %zu's `delayTime` from %f up to default %f (minimum supported: %f).",
                                  i, [delayTime floatValue], kDelayTimeIntervalDefault,
                                  kRCAnimatedDelayTimeIntervalMinimum);
                            delayTime = @(kDelayTimeIntervalDefault);
                        }
        ...
        if (self.frameCount == 0) {
            RCLog(RCAnimatedLogLevelInfo, @"Failed to create any valid frames for GIF with properties %@",
                  imageProperties);
            return nil;
        } else if (self.frameCount == 1) {
            // Warn when we only have a single frame but return a valid GIF.
            RCLog(RCAnimatedLogLevelInfo, @"Created valid GIF but with only a single frame. Image properties: %@",
                  imageProperties);
        } else {
        ...
        if (optimalFrameCacheSize == 0) {
            ...
            if (animatedImageDataSize <= RCAnimatedDataSizeCategoryAll) {
                _frameCacheSizeOptimal = self.frameCount;
            } else if (animatedImageDataSize <= RCAnimatedDataSizeCategoryDefault) {
                ...
                _frameCacheSizeOptimal = RCAnimatedFrameCacheSizeDefault;
            } else {
                // The predicted size exceeds the limits to build up a cache and we go into low memory mode from the
                // beginning.
                _frameCacheSizeOptimal = RCAnimatedFrameCacheSizeLowMemory;
            }
        } else {
        ...
        _weakProxy = (id)[RCWeakProxy weakProxyForObject:self];
...
+ (instancetype)animatedImageWithGIFData:(NSData *)data {
    RCAnimated *animatedImage = [[RCAnimated alloc] initWithAnimatedGIFData:data];
    return animatedImage;
}
...
- (void)addFrameIndexesToCache:(NSIndexSet *)frameIndexesToAddToCache {
    ...
    if (firstRange.length + secondRange.length != self.frameCount) {
        RCLog(RCAnimatedLogLevelWarn, @"Two-part frame cache range doesn't equal full range.");
    }
    ...
    RCAnimated *__weak weakSelf = self;
    dispatch_async(self.serialQueue, ^{
        ...
        void (^frameRangeBlock)(NSRange, BOOL *) = ^(NSRange range, BOOL *stop) {
            ...
            for (NSUInteger i = range.location; i < NSMaxRange(range); i++) {
                ...
                RCLog(RCAnimatedLogLevelVerbose, @"Predrew frame %lu in %f ms for animated image: %@", (unsigned long)i,
...
+ (CGSize)sizeForImage:(id)image {
    ...
    } else if ([image isKindOfClass:[RCAnimated class]]) {
        RCAnimated *animatedImage = (RCAnimated *)image;
        imageSize = animatedImage.size;
    } else {
        // Bear trap to capture bad images; we have seen crashers cropping up on iOS 7.
        RCLog(RCAnimatedLogLevelError, @"`image` isn't of expected types `UIImage` or `RCAnimated`: %@", image);
    }
...
- (NSMutableIndexSet *)frameIndexesToCache {
    ...
    if (self.frameCacheSizeCurrent == self.frameCount) {
        ...
        if ([indexesToCache count] != self.frameCacheSizeCurrent) {
            RCLog(RCAnimatedLogLevelWarn, @"Number of frames to cache doesn't equal expected cache size.");
        }
...
- (void)growFrameCacheSizeAfterMemoryWarning:(NSNumber *)frameCacheSize {
    ...
    RCLog(RCAnimatedLogLevelDebug, @"Grew frame cache size max to %lu after memory warning for animated image: %@",
...
- (void)resetFrameCacheSizeMaxInternal {
    self.frameCacheSizeMaxInternal = RCAnimatedFrameCacheSizeNoLimit;
    RCLog(RCAnimatedLogLevelDebug, @"Reset frame cache size max (current frame cache size: %lu) for animated image: %@",
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification {
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self.weakProxy
                                             selector:@selector(growFrameCacheSizeAfterMemoryWarning:)
                                               object:@(RCAnimatedFrameCacheSizeGrowAfterMemoryWarning)];
    [NSObject cancelPreviousPerformRequestsWithTarget:self.weakProxy
    ...
    RCLog(RCAnimatedLogLevelDebug, @"Attempt setting frame cache size max to %lu (previous was %lu) after memory "
                                   @"warning #%lu for animated image: %@",
          (unsigned long)RCAnimatedFrameCacheSizeLowMemory, (unsigned long)self.frameCacheSizeMaxInternal,
          (unsigned long)self.memoryWarningCount, self);
    self.frameCacheSizeMaxInternal = RCAnimatedFrameCacheSizeLowMemory;
    ...
    if ((self.memoryWarningCount - 1) <= kGrowAttemptsMax) {
        [self.weakProxy performSelector:@selector(growFrameCacheSizeAfterMemoryWarning:)
                             withObject:@(RCAnimatedFrameCacheSizeGrowAfterMemoryWarning)
...
+ (UIImage *)predrawnImageFromImage:(UIImage *)imageToPredraw {
    ...
    if (!colorSpaceDeviceRGBRef) {
        RCLog(RCAnimatedLogLevelError, @"Failed to `CGColorSpaceCreateDeviceRGB` for image %@", imageToPredraw);
    ...
    // Early return on failure!
    if (!bitmapContextRef) {
        RCLog(RCAnimatedLogLevelError, @"Failed to `CGBitmapContextCreate` with color space %@ and parameters (width: "
...
@implementation RCAnimated (Logging)

static void (^_logBlock)(NSString *logString, RCAnimatedLogLevel logLevel) = nil;
static RCAnimatedLogLevel _logLevel;
...
#pragma mark - RCWeakProxy

@interface RCWeakProxy ()

@property (nonatomic, weak) id target;

@end

@implementation RCWeakProxy

#pragma mark Life Cycle

// This is the designated creation method of an `RCWeakProxy` and
// as a subclass of `NSProxy` it doesn't respond to or need `-init`.
+ (instancetype)weakProxyForObject:(id)targetObject {
    RCWeakProxy *weakProxy = [RCWeakProxy alloc];
