...
@protocol RCAnimatedViewDebugDelegate <NSObject>
@optional
- (void)debug_animatedImageView:(RCAnimatedView *)animatedImageView
                waitingForFrame:(NSUInteger)index
                       duration:(NSTimeInterval)duration;
@end
#endif

@interface RCAnimatedView ()
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<RCAnimatedViewDebugDelegate> debug_delegate;
#endif

@end

@implementation RCAnimatedView
...
- (void)setAnimatedImage:(RCAnimated *)animatedImage {
...
- (NSTimeInterval)frameDelayGreatestCommonDivisor {
    // Presision is set to half of the `kRCAnimatedDelayTimeIntervalMinimum` in order to minimize frame dropping.
    const NSTimeInterval kGreatestCommonDivisorPrecision = 2.0 / kRCAnimatedDelayTimeIntervalMinimum;
...
- (void)startAnimating {
    if (self.animatedImage) {
        ...
        if (!self.displayLink) {
            ...
            RCWeakProxy *weakProxy = [RCWeakProxy weakProxyForObject:self];
            self.displayLink = [CADisplayLink displayLinkWithTarget:weakProxy selector:@selector(displayDidRefresh:)];

            [self.displayLink addToRunLoop:[NSRunLoop mainRunLoop] forMode:self.runLoopMode];
        }

        // Note: The display link's `.frameInterval` value of 1 (default) means getting callbacks at the refresh rate of
        // the display (~60Hz).
...
- (void)displayDidRefresh:(CADisplayLink *)displayLink {
    ...
    // Early return!
    if (!self.shouldAnimate) {
        RCLog(RCAnimatedLogLevelWarn, @"Trying to animate image when we shouldn't: %@", self);
        return;
    }
    ...
    if (delayTimeNumber != nil) {
        ...
        if (image) {
            RCLog(RCAnimatedLogLevelVerbose, @"Showing frame %lu for animated image: %@",
            ...
            // While-loop first inspired by & good Karma to:
            // https://github.com/ondalabs/OLImageView/blob/master/OLImageView.m
            ...
            RCLog(RCAnimatedLogLevelDebug, @"Waiting for frame %lu for animated image: %@",
                  (unsigned long)self.currentFrameIndex, self.animatedImage);
#if defined(DEBUG) && DEBUG
