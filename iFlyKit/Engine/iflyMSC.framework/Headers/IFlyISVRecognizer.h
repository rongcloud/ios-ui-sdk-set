 ...
 *
 *  @param length   the serial number's length,length of "98765432" is 8,generally length is 8 and other value is
 * forbidden
 */
 ...
 *
 *  @param pwdt   when pwdt is 1,the function will return chinese text;while pwdt is 2, the funciton will return number
 * serial
 */
 ...
 *  @param ptxt voiceprint txt,only fixed voiceprint and number serial have this,in free voiceprint model this param
 * shall be set nil.
 *  @param vid  another voiceprint type model,user can use this to query or delete model in server can be
 * @"jakillasdfasdjjjlajlsdfhdfdsadff",totally 32 bits;<br>
 *  NOTES:<br>
 *  when vid is not nil,then the server will judge the vid first; while the vid is nil, server can still query or delete
 * the voiceprint model by other params.
 */
- (BOOL)sendRequest:(NSString *)cmd
