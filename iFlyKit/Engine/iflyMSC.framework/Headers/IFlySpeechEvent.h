...
typedef NS_ENUM(NSUInteger, IFlySpeechEventType) {
    ...
    /*!
     * 转写音频文件消息<br>
     * 在录音模式下，成功创建音频文件时返回。可通过onEvent第4个参数data，指定Key为[IFlySpeechConstant
     * IST_AUDIO_PATH],获取音频文件绝对路径.或通过[IFlySpeechTranscripter getParameter:[IFlySpeechConstant
     * IST_AUDIO_PATH]],获取音频文件绝对路径.
     */
    IFlySpeechEventTypeISTAudioFile = 10004,
    /*!
     * 转写已上传字节消息<br>
     * 在消息到达时,通过onEvent的第二个参数arg1,获取已确认上传到服务器的字节数.若当前音频源为非写音频模式,还可通过onEvent
     * 的第三个参数arg2,获取当前所有音频的字节大小.录音模式时，由于所有音频字节大小会变。当停止音频输入后（等待录音时间超时[IFlySpeechConstant
     * SPEECH_TIMEOUT]，或调用[IFlySpeechTranscripter
     * stopTranscripting]），且服务器收到所有音频时，第四个参数data，将包含完成标记的布尔值(true)，可通过data调用指定KEY为KCIFlySpeechEventKeyISTUploadComplete获取。此消息可能多次返回.
     */
